...
from test.level_generate import REAL_LEVELS
...
REAL_LEVELS = REAL_LEVELS[:5]
def eval(model, obs_order, env, device, num_eval=10):
    level = env['level']
    ...
    m3_env = Match3Env(max_step, obs_order=obs_order, levels=Match3Levels([level]))
    for i in range(num_eval):
        current_step = 0
        obs, infos = m3_env.reset()
        while(current_step < max_step):

            action_space = np.array([infos["action_space"]])
            obs_tensor = obs_as_tensor(obs, device)
            action_space = obs_as_tensor(action_space, device)

            actions, values, log_probs = model(obs_tensor, action_space)
            obs, reward, dones, infos = m3_env.step(actions)
            __total_step += 1
...
if __name__ == '__main__':
    ...
    with Pool(processes=num_processes) as pool:
        results = pool.starmap(eval, [(model, args.obs_order, level, device) for level in REAL_LEVELS])
