...
class BaseAlgorithm(ABC):
    ...
    def _excluded_save_params(self) -> List[str]:
        ...
        with this so they can be stored with ``torch.save``.
    ...
    def _get_torch_save_params(self) -> Tuple[List[str], List[str]]:
        ...
        PyTorch ``torch.save``, ``torch.load`` and ``state_dicts`` instead of the default
        ...
        :return:
            List of Torch variables whose state dicts to save (e.g. torch.nn.Modules),
    ...
    def set_parameters(
        ...
        for name in params:
            ...
            if isinstance(attr, torch.optim.Optimizer):
                ...
                # Assume attr is torch.nn.Module
