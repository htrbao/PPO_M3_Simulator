...
class M3Helper:
        ...
        for i in range(self.num_row):
            for j in range(self.num_col):
                ...
                if self.check_legal_pos_to_move(i, j + 1, raw_board):
                    has_match_3 = False
                    for type_c in [0, 1, 2, 3]:
                        if self.check_required_tile_new(
    ...
    def _format_observation(
        ...
        (
            ...
            obs_match_T_new,
            obs_match_5_new,
            obs_legal_action_new,
        ) = self.check_match_new(
            board,
            obs["match_normal"],
            obs["match_2x2"],
            obs["match_4_v"],
        ...
        if not all((
                (obs["match_normal"] == obs_match_normal_new).all(),
                (obs["match_2x2"] == obs_match_2x2_new).all(),
                (obs["match_4_v"] == obs_match_4_v_new).all(),
                (obs["match_4_h"] == obs_match_4_h_new).all(),
                (obs["match_L"] == obs_match_L_new).all(),
                (obs["match_T"] == obs_match_T_new).all(),
                (obs["match_5"] == obs_match_5_new).all(),
                (obs["legal_action"] == obs_legal_action_new).all(),
        )):
            print("Not match!")
